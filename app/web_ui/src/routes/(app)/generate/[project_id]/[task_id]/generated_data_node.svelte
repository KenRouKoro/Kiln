  ...
  <dialog id={`${id}-generate-subtopics`} class="modal">
    <div class="modal-box">
      ...
      <p class="text-sm font-light mb-8">
        ...
        {#if path.length > 0}
          {$_('data_generation.to_path', { values: { path: path.join(" → ") } })}
        {/if}
      </p>
      {#if topic_generating}
      ...
      {:else}
        <div class="flex flex-col gap-2">
          ...
          {/if}
          <div class="flex-grow font-medium">{$_('data_generation.generate_topics')}</div>
          <div class="flex flex-row items-center gap-4 mt-4 mb-2">
            <div class="flex-grow font-medium text-sm">{$_('data_generation.topic_count')}</div>
          ...
          >
            {$_('data_generation.generate_n_topics', { values: { count: num_subtopics_to_generate } })}
          </button>
          <div class="divider">{$_('common.or')}</div>
          <div class="flex flex-col">
            <div class="flex-grow font-medium">{$_('data_generation.custom_topics')}</div>
            <div class="text-xs text-gray-500">{$_('data_generation.comma_separated_list')}</div>
          </div>
          <input
            type="text"
          ...
          <button
            ...
            on:click={add_custom_topics}>{$_('data_generation.add_custom_topics')}</button
          >
        </div>
      {/if}
    </div>
