<script lang="ts">
  ...
  async function request_generate_samples(
    ...
    try {
      if (!model) {
        throw new KilnError($_('generate.samples_modal.no_model_selected'), null)
      }
      const model_provider = model.split("/")[0]
      const model_name = model.split("/").slice(1).join("/")
      if (!model_name || !model_provider) {
        throw new KilnError($_('generate.samples_modal.invalid_model_selected'), null)
      }
      ...
      if (
        ...
        throw new KilnError($_('generate.samples_modal.no_options_returned'), null)
      }
      // Add new samples
...
<dialog id={`${id}-generate-samples`} class="modal">
  <div class="modal-box">
    ...
    <p class="text-sm font-light mb-8">
      ...
      {#if path.length > 0}
        {$_('generate.samples_modal.subtitle_to')} {cascade_mode ? $_('generate.samples_modal.subtitle_each_subtopic') + ' ' : ''}{path.join(" → ")}
      {/if}
    </p>
    ...
    {:else}
      <div class="flex flex-col gap-2">
        <div class="flex flex-row items-center gap-4 mt-4 mb-2">
          <div class="flex-grow font-medium text-sm">{$_('generate.samples_modal.sample_count')}</div>
        ...
        {#if topics_failed_to_generate_count > 0}
          {#if !cascade_mode}
            <div class="text-error font-light text-sm mt-4">
              {$_('generate.samples_modal.failed_to_generate', { values: { topic: topics_failed_to_generate[0].topic.path.join(" → ") } })}
              <div>
          ...
          {:else}
            <div class="text-error font-light text-sm mt-4">
              {$_('generate.samples_modal.topics_failed', { values: { count: topics_failed_to_generate_count } })}
              <button
                class="link"
        ...
        >
          {$_('generate.samples_modal.generate_samples', { values: { count: num_samples_to_generate } })}
          {#if cascade_mode}
