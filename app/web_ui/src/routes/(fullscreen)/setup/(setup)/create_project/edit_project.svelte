...
<div class="flex flex-col gap-2 w-full">
  {#if !created}
    {#if !importing}
      <FormContainer
        submit_label={project.id ? $_('project.update_project') : $_('project.create_project')}
  ...
  {:else if !redirect_on_created}
    {#if importing}
      <h2 class="text-xl font-medium text-center">{$_('project.project_imported')}</h2>
      <p class="text-sm text-center">
        {$_('project.project_imported_message', { values: { path: import_project_path } })}
      </p>
    {:else}
      <h2 class="text-xl font-medium text-center">{$_('project.project_created')}</h2>
      <p class="text-sm text-center">
        {$_('project.project_created_message', { values: { name: project.name } })}
      </p>
    {/if}
  {/if}
</div>
